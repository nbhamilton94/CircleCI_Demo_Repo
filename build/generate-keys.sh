...
base64 -b 64 -i server.key -o server.key.txt
echo "we will now clean up, keys will be deleted"
echo "press enter to continue"
read check4
rm server.csr
rm server.key
echo "certificate and key generateion complete, please add server.crt to your OAuth connected app in salesforce"x